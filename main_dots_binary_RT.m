function main_dots_binary_RT()
% Runs the random-dot motion discrimination task (Shadlen-style dots). Reaction-time version.
...
%     d.setPars(1,'cohStdSet',[0 256]);
    ...
    if query.training==1 && query.expcode==1
        d.setPars(1,'cohSet',[32,64,128,256,512]);
    end
    ...
    for iTr = 1:nTr
        ...
        times.sample('fix_tar_on',   {'type',2,'min',0.3,'max',1,'mean',0.5});
%         times.sample('dots_on',      {'type',2,'min',0.1,'max',0.9,'mean',0.6});
